...
<center> 		<h1 style="font-family: 'Bree Serif',serif;">MODIFICACIÓN DE FOTOS DE ACTIVIDADES</h1></center>
	<form class="needs-validation" novalidate enctype="multipart/form-data"  action="{{route('guarda_fotos_actividades')}}" method="POST">
		{{csrf_field()}}
		<center>
        <div style="align-content: center;">
		<table class="table" style="width: 50%;">
		<tr>
				<td style="text-align: right;">No foto:<br></td>
				<td>
				<input class="form-control" type = 'text' name= 'id_foto_actividad' value="{{$fot->id_foto_actividad}}" readonly="readonly"></td>
			</tr>
<tr>
			<td style="text-align: right;">Selecciona una actividad:<br></td>
				<td>
					<div><select  class="form-control" name = 'id_actividad' id="SearchSelect" required>
						<option disabled="" value="">--Seleccionar actividad--</option>
	    @foreach($id_actividad as $car)
	    <option value="{{ $car->id_actividad }}" @if( $car->id_actividad === $fot->id_actividad )selected='selected' @endif>{{ $car->nombre }}</option>
	    @endforeach
       	</select>
